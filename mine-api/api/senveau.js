// Bu kod Vercel'in sunucusunda çalışacak
...
export default async function handler(request, response) {
  try {
    ...
    const geminiYaniti = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${geminiApiKey}`, {
      method: 'POST',
    ...
    response.status(500).json({ hata: "API işlenirken bir sorun oluştu.", detay: error.message });
  }
}